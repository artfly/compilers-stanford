  ...
  for the location of tokens */
    ...
    */
    ...
    class_list:
      ...
      | class_list error                                    { /* skip until next class */ }
    ...
    class:
      ...
      | CLASS TYPEID error ';'                              { /* skip until next class */}

    
    features:	
        %empty                                              { $$ = nil_Features(); }
      | attr ';'                                            { $$ = single_Features($1); }
      | method ';'                                          { $$ = single_Features($1); }
      | features attr ';'                                   { $$ = append_Features($1, single_Features($2)); }
      | features method ';'                                 { $$ = append_Features($1, single_Features($2)); }
      | error ';'                                           { /* skip until next feature */ }
    ...
    let:
      ...
      | OBJECTID ':' TYPEID ASSIGN expr ',' let             { $$ = let($1, $3, $5, $7); }
      | error ',' let                                       /* skip until next let var */
    ...
    cases:
      ...
      | cases branch ';'                                    { $$ = append_Cases($1, single_Cases($2)); }
    ...
    exprs:
      ...
      | exprs ',' expr                                      { $$ = append_Expressions($1, single_Expressions($3)); }

    block:
        expr ';'                                           { $$ = single_Expressions($1); }
      | block expr ';'                                     { $$ = append_Expressions($1, single_Expressions($2)); }
      | error ';'                                          { /* skip until next expr */ }
    
    expr:
        OBJECTID ASSIGN expr                                { $$ = assign($1, $3); } 
      | expr '@' TYPEID '.' OBJECTID '(' exprs ')'          { $$ = static_dispatch($1, $3, $5, $7); }
      | expr '.' OBJECTID '(' exprs ')'                     { $$ = dispatch($1, $3, $5); }
      | OBJECTID '(' exprs ')'                              { $$ = dispatch(object(idtable.add_string("self")), $1, $3); }
